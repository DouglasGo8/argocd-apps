...
** *Source*: (spec.source) - _WHERE_ is the desired state defined? This normally includes a Git Repo URL, a branch, and a path or Helm Chart
** *Destination*: (spec.destination): _WHERE_ should the manifests from that source be deployed? More specifically, which cluster and namespace?
** *(Optional) Sync Policy*: (spec.syncPolicy): _HOW_ should the deployment be managed? Should it be automated, prune resources, shelf-heal, etc?
